...
<div class="fixed inset-0 bg-transparent backdrop-blur-sm flex items-center justify-center z-50"> <!-- backdrop-blur-sm: agregar para que sea opaco -->
    ...
    <div class="bg-white rounded-lg shadow-lg p-2 md:p-4 w-4xl">
        ...
        <!-- Form Steps -->
        <form action="{{ route('ceremonias.store') }}" id="multi-step-form" method="POST">
            @csrf
            <!-- Step 1 -->
            <div id="step-1" class="step grid grid-cols-[30%_67%] gap-4">
                <div>
                            ...
                            <option disabled {{ old('parroquia') ? '' : 'selected' }}>Seleccione una opción</option>
                                @foreach ($parroquias as $item1)
                                    <option value="{{ $item1->id }}"
                                        {{ old('parroquia') == $item1->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-6">
                        ...
                        <input value="{{ old('fec_ceremonia') }}" type="date" id="fec_ceremonia" name="fec_ceremonia" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:outline-none focus:border-times-hazul w-full p-2">
                    </div>
                </div>
                            ...
                            <option disabled {{ old('sacerdote') ? '' : 'selected' }}>Seleccione una opción</option>
                                @foreach ($sacerdotes as $item2)
                                    <option value="{{ $item2->id }}"
                                        {{ old('sacerdote') == $item2->id ? 'selected' : '' }}>
            ...
            <div id="step-2" class="step hidden">
                <div class="mb-6">
                    ...
                    <textarea name="comentario" id="comentario" cols="20" rows="5" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:outline-none focus:border-times-hazul w-full p-2">{{ old('comentario') }}</textarea>
                </div>
            </div>
...
{{-- PARA CHECKBUTTONS <div class="mb-6">
                    <label for="direccion" class="block mb-2 text-sm font-medium text-gray-900">Observaciones</label>
                    <div class="flex flex-col gap-4 mt-2">
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="observaciones[]" value="opcion1"
                                class="w-4 h-4 text-times-hazul border-gray-300 rounded focus:ring-times-hazul">
                            <span class="text-sm text-gray-900 font-normal">Opción 1</span>
                        </label>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="observaciones[]" value="opcion1"
                                class="w-4 h-4 text-times-hazul border-gray-300 rounded focus:ring-times-hazul">
